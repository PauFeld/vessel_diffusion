...
class GRASSDecoder(nn.Module):
    ...
    def calcularLossAtributo(self, nodo, radio):
        ...
        if nodo is None:
            ...
            l = [self.mseLoss(b.reshape(1,4), gt.reshape(1,4)).mul(1-alfa) for b, gt in zip(radio.reshape(-1,4), nodo.reshape(-1,4))]
            return l
    ...
    def classifyLossEstimator(self, label_vector, original):
        ...
        if original is None:
            ...
            v = torch.stack(v)
            l = [self.ceLoss(b.reshape(1,3), gt.reshape(1,3)).mul(alfa) for b, gt in zip(label_vector.reshape(-1,3), v.reshape(-1,3))]
            return l
